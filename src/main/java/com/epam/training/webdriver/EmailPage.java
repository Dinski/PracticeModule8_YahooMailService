...
public class EmailPage extends BasePage {
    ...
    public WebElement openComposeFolder() {
        try {
            ...
            logger.error("Failed to open compose folder.", e);
            throw e;
        }
        return composeLink;
    }
    ...
    public void composeEmail(Email email){
        try {
            logger.info("Filling receiver: " + email);
            waitForElementToBeVisible(addressee).sendKeys(email.getReceiver());
            waitForElementToBeVisible(subjectField).sendKeys(email.getSubject());
            waitForElementToBeVisible(emailText).sendKeys(email.getContent());
        } catch (Exception e) {
            logger.error("Failed to fill receiver field.", e);
            throw e;
        }
    }
    ...
    public void openDraftEmail() {
        try {
            logger.info("Opening draft email...");
            waitForElementToBeClickable(openDraftEmail);
            openDraftEmail.click();
            logger.info("Draft email opened successfully.");
        } catch (Exception e) {
            logger.error("Failed to open draft email.", e);
            throw e;
        }
    }
    ...
    public void openSentFolder() {
        try {
            ...
            logger.warn("Failed to open sent folder.", e);
            throw e;
        }
    }

    @FindBy(xpath =
            "//div[@data-test-id=\"virtual-list\"]//ul[@role='list']//li[2]//div[@data-test-id='snippet']")
    private WebElement textSentFolder;
